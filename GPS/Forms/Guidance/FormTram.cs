...
namespace AgOpenGPS
{
    public partial class FormTram : Form
    {
        ...
        private void nudTramBoundaryAdd1_Enter(object sender, EventArgs e)
        {
            mf.KeypadToNUD((NumericUpDown)sender, this);
            btnCancel.Focus();
        }

        private void nudTramBoundaryAdd1_ValueChanged(object sender, EventArgs e)
        {
            Properties.Settings.Default.setTram_BoundaryAdd = (double)nudTramBoundaryAdd1.Value;
            Properties.Settings.Default.Save();

            mf.tram.tramBndAdd = Properties.Settings.Default.setTram_BoundaryAdd / 50;
            mf.tram.tramWidth = Properties.Settings.Default.setTram_tramWidth;
            mf.tram.tramWidthBnd = mf.tram.tramWidth + mf.tram.tramBndAdd;
            mf.ABLine.BuildTram();

        }

        private void lblToolWidthHalf_Click(object sender, EventArgs e)
        {
