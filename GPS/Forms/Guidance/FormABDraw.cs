...
namespace AgOpenGPS
{
    public partial class FormABDraw : Form
    {
        ...
        {
            int summe = 0; //anzahl aller punkte berechnen
            for (int b = 0; b < mf.bnd.bndList.Count; b++)
            {
                summe += mf.bnd.bndList[b].fenceLine.Count;
            }
            //int cnt = mf.bnd.bndList[b].fenceLine.Count;
            arr = new vec3[summe * 2];
            int type = 1;

            if (arrType > 0) { type = mf.bnd.bndList.Count; }
            for (int b = 0; b < type; b++) //int b = mf.bnd.bndList.Count - 1; b >= 0; b--
            {
                int cnt = mf.bnd.bndList[b].fenceLine.Count;
                //arr = new vec3[cnt * 2];
                ...
                for (int i = 0; i < cnt; i++)
                {
                    arr[i].easting = mf.bnd.bndList[b].fenceLine[i].easting;
                    arr[i].northing = mf.bnd.bndList[b].fenceLine[i].northing;
                    arr[i].heading = mf.bnd.bndList[b].fenceLine[i].heading;
                }

                for (int i = cnt; i < cnt * 2; i++)
                {
                    arr[i].easting = mf.bnd.bndList[b].fenceLine[i - cnt].easting;
                    arr[i].northing = mf.bnd.bndList[b].fenceLine[i - cnt].northing;
                    arr[i].heading = mf.bnd.bndList[b].fenceLine[i - cnt].heading;
                }
            }
        ...
        private void btnZweitGrenzenAktiv1_Click(object sender, EventArgs e)
        {
            if (arrType == 0)
            {
                arrType = 1;
                btnZweitGrenzenAktiv1.BackColor = Color.LightGreen;
            }
            else
            {
                arrType = 0;
                btnZweitGrenzenAktiv1.BackColor = Color.Gray;
            }
            LoadArr();
        }
