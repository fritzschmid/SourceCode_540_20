...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        private void oglMain_Paint(object sender, PaintEventArgs e)
        {
            ...
            else
            {
                if (isGPSPositionInitialized)
                {
                    ...
                    if (distanzABLine1.isABLineSet)
                    {
                        GL.Color3(colorBerechnungslinie1);
                        distanzABLine1.DrawABLine1();
                    }
                    if (distanzABLine2.isABLineSet)
                    {
                        GL.Color3(colorBerechnungslinie2); 
                        distanzABLine2.DrawABLine2();
                    }
            ...
            if (!tool.isMultiColoredSections)
            {
                if (bnd.bndList.Count > 0)
                {
                    ...
                    if (tool.toolWidth > vehicle.trackWidth)
                    {
                        ...
                        if (TramCount>0 & ((HowMannyPartsAway % TramCount) == 0)){ tram.controlByte += 64 + 128; }
                    }
            ...
            if (fspCnt > 0)
            {
                for (int i = 0; i < fspCnt; i++)
                {
                    ...
                    else if(cPunktFs.punktArr[i].Art == 4)// Brache feldpunkte
                    {
                        ...
                        GL.Vertex3(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, 0);
                        GL.End();
                        font.DrawText3D(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, "&" + cPunktFs.punktArr[i].Nr.ToString(),0.8);
                         // 2 => Sonstige Punkte
                    } else
                    {
                        GL.PointSize(4.0f);
                        ...
                        GL.Vertex3(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, 0);
                        GL.End();
                        font.DrawText3D(cPunktFs.punktArr[i].FeldEast, cPunktFs.punktArr[i].FeldNorth, "&" + cPunktFs.punktArr[i].Nr.ToString(),0.7);
                    }
                }
            }
        }
        private void DrawAntennePoint()
        {
            ...
            {
                string txt = "0";
                double x = oglMain.Width / 2;
                double y = oglMain.Height / 3;// - oglMain.Height / 3;
                if (curve.isBtnCurveOn)
                {
                    if (curve.howManyPathsAway > 0) txt = curve.howManyPathsAway.ToString() + "R";
                    if (curve.howManyPathsAway < 0) txt= curve.howManyPathsAway.ToString() + "L";
                }
                if (ABLine.isBtnABLineOn)
                {
                    if (ABLine.howManyPathsAway > 0) txt= ABLine.howManyPathsAway.ToString() + "R";
                    if (ABLine.howManyPathsAway < 0) txt=ABLine.howManyPathsAway.ToString() + "L";
                }
                GL.Color3(0.1f, 0.9f, 0.0f); //GL.Color4(1.269, 1.25, 1.2510, 0.87);
                font.DrawTextVehicle(x, y, txt, 1);
                font.DrawTextVehicle(0, 0, txt, 1);
            }
            */
            GL.End();
            //********************************************************************************************
        ...
        private void DrawFlags()
        {
            ...
            for (int f = 0; f < flagCnt; f++)
            {
                ...
                if (flagPts[f].color == 0) GL.Color3((byte)255, (byte)0, (byte)flagPts[f].ID);
                if (flagPts[f].color == 1) GL.Color3((byte)255, (byte)127, (byte)flagPts[f].ID);
                if (flagPts[f].color == 2) GL.Color3((byte)255, (byte)255, (byte)flagPts[f].ID);
                if (flagPts[f].color == 3) GL.Color3((byte)127, (byte)255, (byte)flagPts[f].ID);
                if (flagPts[f].color == 4) GL.Color3((byte)0, (byte)255, (byte)flagPts[f].ID);
                //if (flagPts[f].color == 5) GL.Color3((byte)255, (byte)255, (byte)flagPts[f].ID);
        ...
        private void DrawLightBarText()
        {
            ...
            if (ct.isContourBtnOn || ABLine.isBtnABLineOn || curve.isBtnCurveOn)
            {
                ...
                //if (guidanceLineDistanceOff != 32000 && guidanceLineDistanceOff != 32020)
                {
                    ...
                    if (Messpunkt) hede += txt;

                    int center = -(int)(((double)(hede.Length) * 0.5) * 16);
                    font.DrawText(center, 30, hede, 1.8);

                    if (SteerMode <2 ) // anzeigen LookAhead wert fuer Stanley
                    {
                        GL.Color3(0.74f, 0.74f, 0.34f);
                        font.DrawText(center, 75, "StS=" + (gyd.stanleySped.ToString("0.0")), 0.8);
                    }
                    else if (SteerMode ==2 | SteerMode ==3) // anzeigen LookAhead wert fuer PP
                    {
                        GL.Color3(0.34f, 0.74f, 0.74f);
                        font.DrawText(center, 75, "PPla=" + (vehicle.goalPointDistance.ToString("0.0")), 0.8);
                    } else if (SteerMode > 3)// mix
                    {
                        GL.Color3(0.74f, 0.74f, 0.34f);
                        font.DrawText(center, 75, "P=" + (vehicle.goalPointDistance.ToString("0.0")) + " S=" + (gyd.stanleySped.ToString("0.0")), 0.8);
                    }
        ...
        private void DrawCompassText()
        {
            ...
            font.DrawText(center+30, 25, (fixHeading * 57.2957795).ToString("N1"), 1);
        ...
        private void DrawTime()
        {
            ...
            font.DrawText(oglMain.Width/4, oglMain.Height - oglMain.Height/16, System.DateTime.Now.ToLongTimeString() , 1);
        }
