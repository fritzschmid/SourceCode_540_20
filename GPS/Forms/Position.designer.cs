...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        public vec2 steerOnPos = new vec2(0, 0);
        public vec2 steerOffPos = new vec2(0, 0);
        ...
        public double HeadingFilter = Settings.Default.setHeadingFilter;
        ...
        //Fritz
        public int steerGrenzeModus = 0;
        public bool ToolOverlapping = true;
        private bool steerPosOnOff = false;
        ...
        public void UpdateFixPosition()
        {
            ...
            switch (headingFromSource)
            {
                case "Fix":
                    ...
                    //Fritzcode mit IMU fuer Rueckwaertsfahrt
                    {
                        //calculate current heading only when moving, otherwise use last
                        if (ahrs.imuHeading != 99999)
                        {
                        ...
                        if (ahrs.imuHeading != 99999)
                        {
                            ...
                            //use imu as heading when going slow
                            fixHeading = imuCorrected;
                            //fixHeading = (imuCorrected * (100 - HeadingFilter) / 100) + (fixHeading * HeadingFilter / 100);
                ...
                case "Dual":
                    {
                        ...
                        //fixHeading = (glm.toRadians(pn.headingTrueDual) * (100 - HeadingFilter) / 100) + (fixHeading * HeadingFilter / 100);
                        fixHeading = glm.toRadians(pn.headingTrueDual);
                        camHeading = pn.headingTrueDual;//Grad
                        gpsHeading = fixHeading;//Rad
                        ...
                        {
                            //most recent heading
                            newHeading = Math.Atan2(pn.fix.easting - lastReverseFix.easting,
                                                        pn.fix.northing - lastReverseFix.northing);

                            //what is angle between the last reverse heading and current dual heading
                            delta = Math.Abs(Math.PI - Math.Abs(Math.Abs(newHeading - fixHeading) - Math.PI));
                        ...
                        /*
                        if (HeadingFilter > 0 & !isReverse) // heading filtern mit fix2fix
                        {
                            delta = Math.Abs(Math.PI - Math.Abs(Math.Abs(lastHeading - fixHeading) - Math.PI));
                            if (delta > (0.05 + (0.24 / HeadingFilter)))  //0.1 sind ca 6Grad
                            {
                                fixHeading = gpsHeading = (fixHeading * (ahrs.fusionWeight*5) + (newHeading * (1 - (ahrs.fusionWeight*5))));
                            }
                        }
                        */

                        lastHeading = fixHeading; // neues fuer spaeter merken
        ...
        guidanceLineDistanceOff = 32000;
            ...
            if (!vehicle.ast.isInFreeDriveMode)
            {
                ...
                else
                {
                    ...
                    bool drinnenHeadlandSteerOff = !bnd.IsPointInsideHeadArea(steerOffPos);
                    bool drinnenHeadlandSteerOn = !bnd.IsPointInsideHeadArea(steerOnPos);
                    if (drinnenBnd)
                    {
                        if (steerGrenzeModus == 0) // gruen
                        {
                            p_254.pgn[p_254.status] = 7; // bin drinnen setzen
                        }
                        if (steerGrenzeModus==1) //gelb
                        {
                            if(drinnenHeadlandSteerOn & drinnenHeadlandSteerOff) // beide positionen im headland
                            {
                                p_254.pgn[p_254.status] = 3;// wieder wegnehmen
                                steerPosOnOff = false;
                            }
                            if (!drinnenHeadlandSteerOn & !drinnenHeadlandSteerOff) // beide positionen NICHT im headland
                            {
                                p_254.pgn[p_254.status] = 7;// wieder setzen
                                steerPosOnOff = true;
                            }
                            if (steerPosOnOff & drinnenHeadlandSteerOff) // war im feld kommt mit off ins headland
                            {
                                p_254.pgn[p_254.status] = 3;// wieder wegnehmen
                                //steerPosOnOff = false;
                            }
                            if (!steerPosOnOff & !drinnenHeadlandSteerOn) // war im headland kommt mit on ins feld
                            {
                                p_254.pgn[p_254.status] = 7;// wieder setzen
                                //steerPosOnOff = false;
                            }
                            ...
                            //p_254.pgn[p_254.status] = 3;// wieder wegnehmen
                            //if (drinnenHeadlandSteerOff) p_254.pgn[p_254.status] = 7;// wieder setzen
                            //if (drinnenHeadlandSteerOn) p_254.pgn[p_254.status] = 7;// wieder setzen
                        }
                        //if (steerGrenzeModus == 1 & !bnd.isBndBeingMade) //gelb
                       // {
                        //    p_254.pgn[p_254.status] = 7; // bin drinnen setzen
                       // }
                    }
                    ...
                    {
                        p_254.pgn[p_254.status] = 1; // aus dem feld
                    }
                    if (steerGrenzeModus == 2) //rot
                    {
                        p_254.pgn[p_254.status] = 7; // aktivieren
                    }
                    if (p_254.pgn[p_254.status] < 7)
                    {
                        //test ab v13
                        guidanceLineDistanceOff = 32020;
                    }
            ...
            // ins Statusbyte noch Hubwerk und TramlineFS integrieren
            ...
            if (p_239.pgn[p_239.hydLift] == 1) p_254.pgn[p_254.status] += 16;
            if (p_239.pgn[p_239.hydLift] == 2) p_254.pgn[p_254.status] += 32;
            if (TramCount > 0 & ((HowMannyPartsAway % TramCount) == 0)) p_254.pgn[p_254.status] += 64 + 128;
        ...
        private void CalculatePositionHeading()
        {
            ...
            //Steer On position
            steerOnPos.easting = pivotAxlePos.easting + (Math.Sin(fixHeading) * (vehicle.steerOnDist));
            steerOnPos.northing = pivotAxlePos.northing + (Math.Cos(fixHeading) * (vehicle.steerOnDist));
            //Steer Off position
            steerOffPos.easting = pivotAxlePos.easting + (Math.Sin(fixHeading) * (vehicle.steerOffDist));
            steerOffPos.northing = pivotAxlePos.northing + (Math.Cos(fixHeading) * (vehicle.steerOffDist));
