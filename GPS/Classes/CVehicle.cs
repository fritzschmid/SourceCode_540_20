...
namespace AgOpenGPS
{
    public class CVehicle
    {
        ...
        public double steerOnDist = 0;
        public double steerOffDist = 0;
        ...
        public double steerSpeedFaktor = 1;
        private double gyd = 1;
        public double steerStPpMix = 0.5;
        ...
        public double goalPointDistance = 2;
        public double modeXTE = 0;
        private int modeTimeCounter = 0;
        ...
        public double UpdateGoalPointDistance()
        {
           ...
           */ //Ende Original V5
              //Version ab5.6.34
              //double gpd = goalPointLookAhead;
              //goalPointDistance = goalPointLookAhead;
              //goalPointDistance = mf.avgSpeed * goalPointLookAhead * 0.05 * goalPointLookAheadMult;
              //gpd = (mf.avgSpeed * 0.1 * goalPointLookAheadMult) + goalPointLookAhead;
            
            if (mf.avgSpeed>5 & steerSpeedFaktor>1) 
            {
                gyd = ((0.75 + steerSpeedFaktor / 4));
                gyd = 1 + (((mf.avgSpeed-5)/15) * gyd);
            }
            else
            {
                gyd = 1;
            ...
            double mxte = Math.Abs(modeXTE);
            //double fakt = Math.Abs((mxte - xTE) / (xTE));
            if (mf.gyd.distanzHold > mxte)
            {
                if (modeTimeCounter > 10)
                {
                    //ast.modeMultiplier=0.5;
                    //goalPointDistance *= ((ast.modeXTE - xTE) / ast.modeXTE * ast.modeMultiplier) + 1;
                    //gpd *= (mxte - xTE)/(xTE*2) +1; // passt wird zur halben laenge wenn auf linie
                    //gpd = ((fakt) * goalPointLookAheadMult) + ((1 - fakt) * goalPointLookAhead); // wird laenger oder kuerzer mit lookAheadSpeedMulti
                    goalPointDistance = (goalPointLookAheadMult * gyd) * 0.05 + goalPointDistance * 0.95;
                }
                else
                {
                    modeTimeCounter++;
            ...
            {
                modeTimeCounter = 0;
                goalPointDistance = (goalPointLookAhead * gyd) * 0.10 + goalPointDistance * 0.90;
            }
            ...
            if (goalPointDistance < 1) goalPointDistance = 1;
            if (goalPointDistance > 20) goalPointDistance = 20;
            return goalPointDistance;
        }
        ...
        public void DrawVehicle()
        {
            ...
            if (mf.steerGrenzeModus == 1)
            {
                //SteerOnPunkt
                GL.PointSize(6.0f);
                GL.Color3((byte)40, (byte)200, (byte)40);
                GL.Begin(PrimitiveType.Points);
                GL.Vertex2(0, steerOnDist);
                GL.End();
                //SteerOffPunkt
                GL.PointSize(6.0f);
                GL.Color3((byte)200, (byte)40, (byte)40);
                GL.Begin(PrimitiveType.Points);
                GL.Vertex2(0, steerOffDist);
                GL.End();
            }
            //AntennePunkt
            GL.PointSize(4.0f);
