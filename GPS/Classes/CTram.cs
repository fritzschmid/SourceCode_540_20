...
namespace AgOpenGPS
{
    ...
    public class CTram
    {
        ...
        public double tramBndAdd;
        public double tramWidthBnd;
        ...
        public CTram(FormGPS _f)
        {
            //constructor
            mf = _f;
            tramBndAdd = Properties.Settings.Default.setTram_BoundaryAdd / 50; //cm /100*2
            tramWidth = Properties.Settings.Default.setTram_tramWidth;
            tramWidthBnd = tramWidth + tramBndAdd;
        ...
        private void CreateBndInnerTramTrack()
        {
            ...
            double distSq = ((tramWidthBnd * 0.5) + halfWheelTrack) * ((tramWidthBnd * 0.5) + halfWheelTrack) * 0.999;
            ...
            for (int i = 0; i < ptCount; i++)
            {
                ...
                pt3.easting = mf.bnd.bndList[0].fenceLine[i].easting -
                    (Math.Sin(glm.PIBy2 + mf.bnd.bndList[0].fenceLine[i].heading) * (tramWidthBnd * 0.5 + halfWheelTrack));

                pt3.northing = mf.bnd.bndList[0].fenceLine[i].northing -
                    (Math.Cos(glm.PIBy2 + mf.bnd.bndList[0].fenceLine[i].heading) * (tramWidthBnd * 0.5 + halfWheelTrack));
