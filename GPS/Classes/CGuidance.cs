...
namespace AgOpenGPS
{
    public class CGuidance
    {
        ...
        // Winkel und Distanz fuer Mix
        public double lwS, lwP, dvlS, dvlP;
        public double stanleySped;
        //derivative counter
        private int counter;
        private double sped;
        private double XTEc;
        private double gyd = 1;
        public double distanzHold = 0.20;// abstand von linie ab der anders gerechnet werden soll
        public double testRegler = 5;
        public double distanzLine = 0.0;// abstand distanz zu linie abs();
        ...
        private void DoSteerAngleCalc()
        {
            ...
            steerHeadingError *= mf.vehicle.stanleyHeadingErrorGain;// original
            
            sped = Math.Abs(mf.avgSpeed);

            if (sped > 1) sped = (1 + 0.2) * (sped - 1); //sped = 1 + 0.277 * (sped - 1);
            else sped = 1;

            if ((sped > 1) && (mf.vehicle.steerSpeedFaktor > 1) & (distanceFromCurrentLineSteer<0.15))
            {
                gyd = ((0.875 + mf.vehicle.steerSpeedFaktor /8));
                //gyd = 1 + ((sped - 4) * gyd);
                sped*=gyd;
            }
            else
            ...
            XTEc = Math.Atan((distanceFromCurrentLineSteer * mf.vehicle.stanleyDistanceErrorGain)
                / (sped));
            stanleySped = sped;// * mf.vehicle.stanleyDistanceErrorGain/5;
            ...
            if (mf.pn.speed > mf.startSpeed & mf.isAutoSteerBtnOn) //&& Math.Abs(derivativeDistError) < 3 &&
            {
                if (mf.SteerMode==1 & (Math.Abs(pivotDistanceError) < 0.25)) // FRITZ
                {
                    //if over the line heading wrong way, rapidly decrease integral
                    if ((inty < 0 && distanceFromCurrentLinePivot < 0) | (inty > 0 && distanceFromCurrentLinePivot > 0))//||
                    {
                        inty += pivotDistanceError * mf.vehicle.stanleyIntegralGainAB * -0.31;
                    }
                    else
                    {
                        inty += pivotDistanceError * mf.vehicle.stanleyIntegralGainAB * -0.031;
                    }
                }
                else if(mf.SteerMode==0 & (Math.Abs(pivotDistanceError) < 0.25))//Original
                {
                    //if over the line heading wrong way, rapidly decrease integral
            ...
            if (mf.SteerMode > 1) inty = 0; // bei SteerMode ==4  Inty deaktivieren da PP mitmischt
            ...
            dvlS = distanceFromCurrentLinePivot;
            lwS = steerAngleGu;
