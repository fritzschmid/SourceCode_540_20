    ...
    public class CYouTurn
    {
        ...
        public bool BuildABLineDubinsYouTurn(bool isTurnRight)
        {
            ...
            if (youTurnPhase == 0)
            {
                ...
                if (Properties.Settings.Default.setUturnAutoRadius) CDubins.turningRadius = vehicleTurnRadius;// & (mf.vehicle.minTurningRadius< vehicleTurnRadius)
                ...
                if (Properties.Settings.Default.setUturnAutoRadius) turnRadius = turnOffset/2;// neu
                ...
                head -= Math.PI;
                if (head < -Math.PI) head += (Math.PI * 2);// glm.twoPI;
                if (head > Math.PI) head -= (Math.PI * 2);// glm.twoPI;

                if (!Properties.Settings.Default.setUturnAutoRadius)//((mf.vehicle.minTurningRadius * 2.0) < turnOffset)
                {
        ...
        public void BuildManualYouTurn(bool isTurnRight, bool isTurnButtonTriggered)
        {
            ...
            CDubins dubYouTurnPath = new CDubins();
            CDubins.turningRadius = mf.vehicle.minTurningRadius;
            if (Properties.Settings.Default.setUturnAutoRadius) CDubins.turningRadius = turnOffset/2;
