    ...
    public class CABCurve
    {
        ...
        public void GetCurrentCurveLine(vec3 pivot, vec3 steer)
        {
            ...
            if (ptCount > 0)
            {
                ...
                else if (mf.SteerMode == 0)//Stanley Original
                {
                    mf.gyd.StanleyGuidanceCurve0(pivot, steer, ref curList);
                }
                else if (mf.SteerMode == 1)//Stanley Fritz
                {
                    mf.gyd.StanleyGuidanceCurve1(pivot, steer, ref curList);
                }
                else if (mf.SteerMode == 2)// Pure Pursuit Original------------------------------------------
                {
                ...
                { //SteerMode==3 PP Fritz ******************************************************************************************************************************************
                    ...
                    //how far from current AB Line is fix
                    //distanceFromCurrentLinePivot = ((dz * pivot.easting) - (dx * pivot.northing) + (curList[B].easting * curList[A].northing) - (curList[B].northing * curList[A].easting)) / Math.Sqrt((dz * dz) + (dx * dx));
                    distanceFromCurrentLinePivot = ((dz * steer.easting) - (dx * steer.northing) + (curList[B].easting * curList[A].northing) - (curList[B].northing * curList[A].easting)) / Math.Sqrt((dz * dz) + (dx * dx));
                ...
                else // StPpMix SteerMode ==4 #####################################################################################################
                {
                    //SteerMode==3 PP Fritz berechnue fuer Mix
                    //find the closest 2 points to current fix
                    for (int t = 0; t < ptCount; t++)
                    {
                        dist = glm.DistanceSquared(pivot, curList[t]);
                    ...
                    /*
                                        ...
                                        */
                    inty = 0; // bei SteerMode==4 auf null setzen
                    // ** Pure pursuit ** - calc point on ABLine closest to current position
                    double U = (((pivot.easting - curList[A].easting) * dx)
                                + ((pivot.northing - curList[A].northing) * dz))
                                / ((dx * dx) + (dz * dz));
                    ...
                    goalPointDistanceSquared = (goalPointDistanceSquared * (1 - mf.gyd.testRegler)) + (goalPointDistance * mf.gyd.testRegler);
                    //calculate the the delta x in local coordinates and steering angle degrees based on wheelbase
                    //double localHeading = glm.twoPI - mf.fixHeading;
                   ...
                   + ((goalPointCu.northing - steer.northing) * Math.Sin(localHeading))) * mf.vehicle.wheelbase / goalPointDistanceSquared));
                    ...
                    //steerAngleCu += inty;//Fritz bei Steerm´Mode==4 kein inty da stanley reinmischt
                    ...
                    {
                        steerAngleCu = mf.gyd.lwP * mf.vehicle.steerStPpMix + mf.gyd.lwS * (1 - mf.vehicle.steerStPpMix);
                    }
                    else
                    {
                        steerAngleCu = mf.gyd.lwP;
                    }
