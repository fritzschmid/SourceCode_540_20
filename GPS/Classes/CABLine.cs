...
namespace AgOpenGPS
{
    public class CABLine
    {
        ...
        public double distanceFromCurrentLinePivot, distanceFromCurrentLinePivotOld;
        ...
        public double RechneDistanzVonRefLine()
        {
            double dx, dy;
            double distanceFromDesLine;
            //x2-x1
            dx = desP2.easting - desP1.easting;
            //z2-z1
            dy = desP2.northing - desP1.northing;

            distanceFromDesLine = ((dy * mf.pivotAxlePos.easting) - (dx * mf.pivotAxlePos.northing) + (desP2.easting
                                    * desP1.northing) - (desP2.northing * desP1.easting))
                                        / Math.Sqrt((dy * dy) + (dx * dx));
            return distanceFromDesLine;
        }
        ...
        public void GetCurrentABLine(vec3 pivot, vec3 steer)
        {
...

            {
                ...
                mf.vehicle.modeXTE = distanceFromCurrentLinePivot;
            ...
            else if (mf.SteerMode == 3)// Pp3 SteerMode==3 // Fritz
                                       //************************************************************************************************************************************************************
            {
                //get the distance from currently active AB line
                //x2-x1
                ...
                if (mf.vehicle.purePursuitIntegralGain != 0 && !mf.isReverse)
                {
...

                    {
                        ...
                        if ((inty < 0 && distanceFromCurrentLinePivot < 0) || (inty > 0 && distanceFromCurrentLinePivot > 0))
                        {
                            inty += pivotDistanceError * mf.vehicle.purePursuitIntegralGain * -0.4;
                        }
                        else
                        {
                            if (Math.Abs(distanceFromCurrentLinePivot) > 0.25)
                            {
                                inty += pivotDistanceError * mf.vehicle.purePursuitIntegralGain * -0.2;
                                if (inty > 5.2) inty = 5.2;
                                else if (inty < -5.2) inty = -5.2;
                            }
                        }
                    }
                    else inty *= 0.95;
...
*/
            ...
            {       //    PpStanlexMix Fritz #########################################################################################################
                ...
                //distanceFromCurrentLinePivotOld = distanceFromCurrentLinePivot;
                //how far from current AB Line is fix
                //distanceFromCurrentLinePivot = ((dy * pivot.easting) - (dx * pivot.northing) + (currentABLineP2.easting * currentABLineP1.northing) - (currentABLineP2.northing * currentABLineP1.easting)) / Math.Sqrt((dy * dy) + (dx * dx));
                distanceFromCurrentLinePivot = ((dy * steer.easting) - (dx * steer.northing) + (currentABLineP2.easting * currentABLineP1.northing) - (currentABLineP2.northing * currentABLineP1.easting)) / Math.Sqrt((dy * dy) + (dx * dx));

                //distanceFromCurrentLinePivot = distanceFromCurrentLinePivot * 0.6 + distanceFromCurrentLinePivotOld * 0.4;
               /*
                //integral slider is set to 0
                if (mf.vehicle.purePursuitIntegralGain != 0 && !mf.isReverse)
                {
...
*/
                ...
                //calc "D" the distance from pivot axle to lookahead point
                double goalPointDistanceDSquared = glm.DistanceSquared(goalPointAB.northing, goalPointAB.easting, pivot.northing, pivot.easting);
                //double goalPointDistanceDSquared = glm.DistanceSquared(goalPointAB.northing, goalPointAB.easting, steer.northing, steer.easting);
                
                goalPointDistanceDSquared = (goalPointDistanceDSquared * (1- mf.gyd.testRegler)) + (goalPointDistance * mf.gyd.testRegler);
...
*/
                ...
                //if (Math.Abs(distanceFromCurrentLinePivot) > Math.Abs(distanceFromCurrentLinePivotOld)) steerAngleAB /= 3;

                if (mf.ahrs.imuRoll != 88888)
                    steerAngleAB += mf.ahrs.imuRoll * -mf.gyd.sideHillCompFactor;

                //steerAngleAB += inty;// fs +=inty; //Fritz bei Steerm´Mode==4 kein inty da stanley reinmischt
                ...
                mf.gyd.dvlP = distanceFromCurrentLinePivot; // distanz diese berechnung
                mf.gyd.lwP = steerAngleAB;
                // noch stanley berechnen
                mf.gyd.StanleyGuidanceABLine1(currentABLineP1, currentABLineP2, pivot, steer);

                //Mittelwert berechnen
                distanceFromCurrentLinePivot = mf.gyd.dvlP * mf.vehicle.steerStPpMix + mf.gyd.dvlS * (1- mf.vehicle.steerStPpMix);
                mf.gyd.distanzLine= Math.Abs(distanceFromCurrentLinePivot);
                if (mf.gyd.distanzLine< mf.gyd.distanzHold)
                {
                    steerAngleAB = mf.gyd.lwP * mf.vehicle.steerStPpMix + mf.gyd.lwS * (1 - mf.vehicle.steerStPpMix);
                }
                else
                {
                    steerAngleAB = mf.gyd.lwP;
                }
        ...
        public void DrawABLine1()
        {
            ...
            */
            ...
            GL.LineWidth(lineWidth);
            GL.Enable(EnableCap.LineStipple);
            GL.LineStipple(1, 0x7F00);//strichliert
            GL.Begin(PrimitiveType.Lines);
            //GL.Color3(0.99f, 0.66f, 0.26f);
            GL.Vertex3(desP1.easting, desP1.northing, 0);
            GL.Vertex3(desP2.easting, desP2.northing, 0);
            GL.End();
            GL.Disable(EnableCap.LineStipple);
            /*
            //draw current AB Line
